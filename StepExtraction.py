...
def getminimas(dataMatrix, Sensor=[181]):
    ...
    return argrelmin(Filtered[:,Sensor],order=20)

def getmaximas(dataMatrix, Sensor=[181]):
    signal = dataMatrix[:,Sensor[0]]
    maxAbsValue, maxAbsFreq = FourierTransformation.maxAbsFreq(signal)
    print maxAbsFreq
    Filtered = FeatureKonstruktion.filter(dataMatrix,Sensor,maxAbsFreq)
    return argrelmax(Filtered[:,Sensor],order=20)
